...
procedure TForm2.btmResultClick(Sender: TObject);
  var
    xidade : integer ;
    xnome, xsexo, xcondição : string;

  begin

    xnome := inputbox('Informar','Informe um nome','');
    xsexo := (inputbox('Informar','Informe o Sexo',''));
    xcondição := (inputbox('Informar','Informe a condição de saúde',''));
  ...
  if AnsiLowerCase(xcondição) = 'apto para servir' then
    FxcontadorS   := FxcontadorS +1
  else if AnsiLowerCase(xcondição) = 'inapto para servir' then
    FxcontadorS   := FxcontadorS +1;
